...
import { z } from "zod";
import type { StockRequestWithItems, StockItem, Order } from "@shared/schema";
...
function makeOrderSchema(maxQty: number) {
  return z.object({
    ...
    orderedQuantity: z.coerce
      ...
      .max(maxQty, `残数 ${maxQty} を超えて発注できません`),
...
export default function RequestDetail() {
  ...
  const { data: request, isLoading } = useQuery<StockRequestWithItems>({
  ...
  const { data: orders } = useQuery<Order[]>({
    queryKey: ["/api/requests", id, "orders"],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/requests/${id}/orders`);
      return res.json();
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (pin: string) => {
      const res = await apiRequest("DELETE", `/api/requests/${id}?pin=${pin}`);
  ...
  const handlePinSubmit = async () => {
    try {
      const res = await apiRequest("POST", `/api/requests/${id}/verify-pin`, { pin: pinInput });
  ...
  const isExpired = request.postDeadline
    ? !isAfter(parseISO(request.postDeadline), new Date())
    : false;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <Link href="/">
          <a className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors" data-testid="link-back">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => handlePinAction("delete")}
        ...
        <div className="bg-card border-2 border-primary/30 rounded-lg p-5 shadow-lg">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-foreground">
              {pinAction === "edit" ? "編集" : "削除"}にはPINが必要です
            </h3>
          </div>
          ...
          <div className="flex gap-2 items-start">
            <div>
              <Input
                ...
                data-testid="input-pin-dialog"
              />
              {pinError && <p className="text-xs text-destructive mt-1">{pinError}</p>}
            </div>
            <Button onClick={handlePinSubmit} data-testid="button-pin-submit">認証</Button>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className={`px-5 py-4 border-b border-border ${statusConfig.bg}`}>
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {isExpired && (
              <span className="text-xs text-destructive font-medium border border-destructive/30 rounded px-1.5 py-0.5">
                掲示期限切れ
              </span>
            )}
          </div>
        ...
        <div className="px-5 py-4 grid grid-cols-2 gap-3 text-sm border-b border-border">
          <div className="flex items-center gap-2 text-muted-foreground">
            <MapPin size={13} />
            <span>拠点: <strong className="text-foreground">{request.requesterBase}</strong></span>
          </div>
          <div className="flex items-center gap-2 text-muted-foreground">
            <User size={13} />
            <span>依頼者: <strong className="text-foreground">{request.requesterName}</strong></span>
          </div>
          {request.requesterExtension && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Phone size={13} />
              <span>内線: <strong className="text-foreground">{request.requesterExtension}</strong></span>
            </div>
          )}
          <div className="flex items-center gap-2 text-muted-foreground">
          ...
          {(request as any).requesterEmail && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Mail size={13} />
              <span>メール: <strong className="text-foreground">{(request as any).requesterEmail}</strong></span>
            </div>
          )}
          {request.note && (
            <div className="col-span-2 text-muted-foreground text-xs">
              備考: <span className="text-foreground">{request.note}</span>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-3 bg-muted/30 flex items-center gap-3 flex-wrap">
          <span className="text-xs text-muted-foreground">
            ステータスは発注状況に応じて自動更新されます（受付中 → 対応中 → 完了）
          </span>
        </div>
      </div>
...
// ========== 商品1行 + 発注フォーム ==========
function ItemBlock({
  item, index, request, orders,
}: {
  item: StockItem;
  index: number;
  request: StockRequestWithItems;
  orders: Order[];
}) {
  const { toast } = useToast();
  ...
  const orderMutation = useMutation({
    mutationFn: async (data: OrderFormValues) => {
      const res = await apiRequest("POST", "/api/orders", { ...data, orderDate: today });
      return res.json();
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["/api/requests", String(request.id), "orders"] });
      queryClient.invalidateQueries({ queryKey: ["/api/requests"] });
      queryClient.invalidateQueries({ queryKey: ["/api/requests", String(request.id)] });
      
      if (result.autoCompleted) {
        toast({ title: "発注しました — 全数量の発注が完了し、依頼ステータスが「完了」になりました" });
      } else {
        toast({ title: "発注しました" });
      }
  ...
  return (
    <div className="px-5 py-4" data-testid={`item-block-${item.id}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-start gap-2 min-w-0">
          <span className="mt-0.5 text-xs font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded shrink-0">
            {index + 1}
          </span>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-xs mb-1">
          ...
          <span className={`font-semibold ${isFulfilled ? "text-green-600 dark:text-green-400" : remaining < item.quantity * 0.3 ? "text-orange-500" : "text-primary"}`}>
            残: {remaining.toLocaleString()}{item.unit}
            {isFulfilled && " ✓完了"}
          </span>
        </div>
        <div className="w-full h-2 bg-muted rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all ${
              isFulfilled ? "bg-green-500" : pct > 70 ? "bg-orange-400" : "bg-primary"
            }`}
      ...
      {itemOrders.length > 0 && (
        <div className="mb-3 space-y-1">
          {itemOrders.map((o) => (
            <div key={o.id} className="flex items-center gap-3 text-xs bg-muted/40 rounded px-3 py-1.5" data-testid={`order-row-${o.id}`}>
              ...
              {(o as any).ordererExtension && (
                <span className="text-muted-foreground flex items-center gap-0.5"><Phone size={10} />{(o as any).ordererExtension}</span>
              )}
      ...
      {isFulfilled ? (
        ...
        <Button
          ...
          data-testid={`button-open-order-${item.id}`}
        >
          <ShoppingCart size={14} />
          発注する（残: {remaining.toLocaleString()}{item.unit}）
        </Button>
      ) : (
        <OrderForm
          item={item}
          ...
          requestId={request.id}
          form={form}
          onSubmit={(d) => orderMutation.mutate(d)}
...
// ========== 発注フォーム（3項目のみ） ==========
function OrderForm({
  item, remaining, form, onSubmit, onClose, isPending,
}: {
  item: StockItem;
  remaining: number;
  requestId: number;
  form: ReturnType<typeof useForm<OrderFormValues>>;
  onSubmit: (d: OrderFormValues) => void;
  onClose: () => void;
  isPending: boolean;
}) {
  const qty = form.watch("orderedQuantity");
  const afterRemaining = Math.max(0, remaining - (Number(qty) || 0));

  return (
    <div className="border border-primary/30 rounded-lg p-4 bg-primary/5 mt-1" data-testid="order-form">
      ...
      <div className="flex items-center gap-2 mb-4 text-sm bg-background rounded px-3 py-2 border border-border">
        ...
        {Number(qty) > 0 && (
          <>
            ...
            <span className={`font-bold tabular-nums ${afterRemaining === 0 ? "text-green-600 dark:text-green-400" : "text-foreground"}`}>
              発注後残数: {afterRemaining.toLocaleString()} {item.unit}
            </span>
          </>
        )}
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem>
                <FormLabel>
                  ...
                  <span className="text-xs text-muted-foreground ml-2 font-normal">
                    最大 {remaining.toLocaleString()}{item.unit}
                  </span>
                </FormLabel>
                <FormControl>
                  <Input {...field} type="number" min={1} step="any" className="w-48" data-testid={`input-order-qty-${item.id}`} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} placeholder="例: 倉庫担当 鈴木" data-testid={`input-orderer-${item.id}`} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            control={form.control}
            name="ordererExtension"
            render={({ field }) => (
              <FormItem>
                <FormLabel>内線番号（任意）</FormLabel>
                <FormControl>
                  <Input {...field} value={field.value ?? ""} placeholder="例: 1234" className="w-48" data-testid={`input-orderer-ext-${item.id}`} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea {...field} value={field.value ?? ""} placeholder="分納対応など特記事項があれば記入" rows={2} data-testid={`input-order-note-${item.id}`} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <Button type="submit" disabled={isPending} className="w-full" data-testid={`button-order-submit-${item.id}`}>
            <ShoppingCart size={14} className="mr-1.5" />
            {isPending ? "発注中..." : "発注を確定する"}
          </Button>
        </form>
      </Form>
    </div>
